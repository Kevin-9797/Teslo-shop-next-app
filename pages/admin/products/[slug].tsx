...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onChangeSize = (size: string) => {
    ...
    if (currentSizes.includes(size)) {
      return setValue(
        ...
        { shouldValidate: true }
      ); //shouldValidate es para que lo valide y renderize al instante
    }
    setValue("sizes", [...currentSizes, size], { shouldValidate: true });
  };

  const onChangeFile = async( {target}:ChangeEvent<HTMLInputElement>) => {
  ...
  const onSubmitForm = async (formData: FormData) => {
    if (formData.images.length < 2) {
      return alert("You have to upload at least 2 images ");
    }
    setIsSaving(true);
    try {
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <TextField
              ...
              {...register("inStock", {
                required: "Zone required",
                minLength: { value: 0, message: "Mín 0 characters" },
              })}
